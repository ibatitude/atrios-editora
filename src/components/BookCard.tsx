    ...
    >
      ...
      <div className="relative bg-[#F5F1EB] p-6 flex items-center justify-center overflow-hidden min-h-[280px]">
        ...
        <div
          ...
          className="cursor-pointer relative z-10 transition-transform duration-300 group-hover:scale-105"
        >
      ...
      <div className="p-5 flex-grow flex flex-col justify-between bg-white">
        <div>
          ...
          <h3
            onClick={() => onOpenDetails(book)}
            className="font-serif font-bold text-lg text-[#0F2C2C] leading-snug hover:opacity-75 cursor-pointer line-clamp-1 transition-opacity"
          >
            {book.title}
          </h3>

          <p className="text-xs text-[#0F2C2C]/70 mt-0.5 mb-2.5">
            Por <span className="text-[#0F2C2C] font-semibold">{book.author}</span>
          </p>
        ...
        <div className="pt-3 border-t border-[#0F2C2C15] flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-[#0F2C2C] bg-[#F5F1EB] hover:bg-[#E8E2D9] rounded-sm transition-colors cursor-pointer flex items-center gap-1"
            ...
            <button
              onClick={() => onQuickBuy ? onQuickBuy(book) : onOpenDetails(book)}
              id={`book-buy-btn-${book.id}`}
              className="p-2 text-white bg-[#0F2C2C] hover:bg-[#1a4040] rounded-sm transition-colors cursor-pointer"
