...
import { Author, Book, PageId } from '../types';
import {
  Feather,
  BookOpen,
  MapPin,
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
                  ...
                  <div className="space-y-1.5 flex-grow">
                    <h3
                      ...
                      className="font-serif font-bold text-xl sm:text-2xl text-[#0F2C2C] hover:opacity-80 cursor-pointer transition-opacity"
                    >
                      {author.name}
                    </h3>
                ...
                <div className="space-y-2 pt-2 border-t border-[#0F2C2C15]">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {authorBooks.map((b) => (
                      <button
                        key={b.id}
                        onClick={() => onOpenBookModal(b)}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-sm bg-[#F5F1EB] hover:bg-[#0F2C2C] hover:text-white text-[#0F2C2C] text-xs font-bold transition-colors cursor-pointer group"
                      >
                ...
                <div className="pt-2 flex justify-end">
                  <button
                    onClick={() => onOpenAuthorModal(author)}
                    className="inline-flex items-center gap-1.5 text-xs font-bold text-[#0F2C2C] hover:opacity-80 transition-opacity cursor-pointer uppercase tracking-wider"
                  >
      ...
      <section className="bg-[#0F2C2C] text-[#FDFBF7] rounded-sm p-8 sm:p-12 border border-[#0F2C2C] shadow-lg text-center max-w-4xl mx-auto space-y-6">
        ...
        <div className="pt-2">
          <button
            ...
            className="px-8 py-4 bg-[#D9D1C5] hover:bg-[#E8E2D9] text-[#0F2C2C] font-bold text-xs uppercase tracking-widest rounded-sm shadow-md transition-all inline-flex items-center gap-2 cursor-pointer"
          >
